var LiveSearch = {
  ...
  addCallback: function (e, r) {
    return (
      void 0 === this.callbacks[e] && (this.callbacks[e] = []),
      this.callbacks[e].push(r)
    );
  },
  invokeCallbacks: function (e, r) {
    var s;
    if (void 0 !== this.callbacks[e])
      for (s in this.callbacks[e]) r = this.callbacks[e][s](r);
  },
  init: function () {
    (LiveSearch.searchBoxes = jQuery("input")
      .filter("[name='s']")
      .not(".no-livesearch")),
      LiveSearch.searchBoxes.keyup(LiveSearch.handleKeypress),
      LiveSearch.searchBoxes.focus(LiveSearch.hideResults),
      LiveSearch.searchBoxes.outerHeight ||
      ...
      (LiveSearch.compiledResultTemplate = _.template(
        DavesWordPressLiveSearchConfig.resultTemplate
      )),
  ...
  positionResults: function () {
    ...
    if (s && r.size() > 0) {
      ...
      switch (
        ...
        s.css("top", a.top),
        s.css("display", "block"),
        DavesWordPressLiveSearchConfig.resultsDirection)
      ) {
        case "up":
          e = a.top - s.height();
          break;
  ...
  handleAJAXResults: function (e) {
    ...
    if ((LiveSearch.activeRequests.pop(), e)) {
      if (
        ...
        resultsSearchTerm != jQuery("input:focus").first().val())
      )
        return void (
          0 === LiveSearch.activeRequests.length && LiveSearch.removeIndicator()
        );
      var s = jQuery("#dwls_search_results")
      ...
      if (0 === e.results.length)
        LiveSearch.hideResults(), LiveSearch.invokeCallbacks("ZeroResults");
      else
        LiveSearch.invokeCallbacks("BeforeDisplayResults"),
          ...
          jQuery("#dwls_search_results").size() > 0
          ...
          jQuery("#dwls_search_results li").bind("click.dwls", function (e) {
            window.location.href = jQuery(this)
              .find("a.daves-wordpress-live-search_title")
              .attr("href");
          }),
  ...
  runQuery: function (e) {
    var r,
      s,
      a = jQuery("input:focus"),
      i = a.val();
    if ("" === i || i.length < DavesWordPressLiveSearchConfig.minCharsToSearch)
      LiveSearch.hideResults(), LiveSearch.removeIndicator();
    else {
      for (
        LiveSearch.displayIndicator();
        LiveSearch.activeRequests.length > 0;

      )
        (s = LiveSearch.activeRequests.pop()).abort();
      var t = {},
      ...
      (t.action = "dwls_search"),
        ((s = jQuery.get(
          ...
          "json"
        )).fail = LiveSearch.ajaxFailHandler),
        LiveSearch.activeRequests.push(s);
    }
  },
  ajaxFailHandler: function (e) {
    console.log(
      "Dave's WordPress Live Search: There was an error retrieving or parsing search results"
    ),
  ...
  displayIndicator: function () {
    if (0 === jQuery(".search_results_activity_indicator").size()) {
      var e = jQuery("input:focus").first(),
        r = e.offset();
      jQuery("body").append(
        '<span id="search_results_activity_indicator" class="search_results_activity_indicator" />'
      );
      var s = { outer: Math.ceil((0.9 * e.height()) / 2) };
      ...
      jQuery(".search_results_activity_indicator").css("left", i),
